...
@section('menuHujanJateng', 'active')

@section('content')
<div class="text-center mt-3 mb-2   ">
    <h5>Data Hujan Wilayah Jawa Tengah</h5>
</div>
<form action="{{ route('hujan.filterJateng') }}">
...
<script>
    const selectElement = document.getElementById('kabupaten');
        ...
        selectElement.addEventListener('change', function() {
            ...
            if (selectedValue === '') {
                window.location.href = '{{ route("hujan.jateng") }}';
            } else {
                //const kabupaten = selectedValue.replace(/ /g, '+');
                window.location.href = `{{ route("hujan.filterJateng") }}?kabupaten=${selectedValue}`;
            }
        });
</script>
@endpush
